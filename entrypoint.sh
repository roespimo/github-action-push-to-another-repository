...
while [ $retry_count -lt $MAX_RETRIES ]; do
  echo "[+] Attempt #$((retry_count + 1)) - Action start"
...
git push "$GIT_CMD_REPOSITORY" --set-upstream "$TARGET_BRANCH"
  ...
  # Guarda el código de salida del script
  script_exit_code=$?

  # Verifica si el script se ejecutó correctamente
  if [ $script_exit_code -eq 0 ]; then
    echo "[+] Script executed successfully"
    break  # Sal del bucle si el script se ejecutó correctamente
  else
...
# Comprueba si se ha agotado el número de reintentos y el script sigue fallando
if [ $retry_count -eq $MAX_RETRIES ] && [ $script_exit_code -ne 0 ]; then
  echo "::error::Script failed after $MAX_RETRIES retries"
  exit 1
fi
