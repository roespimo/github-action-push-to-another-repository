...
set -e  # si un comando falla, detiene la ejecución
set -u  # el script falla si intenta acceder a una variable no definida
MAX_RETRIES=5
RETRY_DELAY=10 
...
# Verifica si hay acceso (potencialmente) al repositorio de destino
# y configura git (con la variable GIT_CMD) y GIT_CMD_REPOSITORY
if [ -n "${SSH_DEPLOY_KEY:=}" ]
then
	echo "[+] Using SSH_DEPLOY_KEY"

	# Inspirado en https://github.com/leigholiver/commit-with-deploy-key/blob/main/entrypoint.sh , gracias!
...
# Solución temporal para https://github.com/cpina/github-action-push-to-another-repository/issues/103
...

{
    ...
    then
        # Se clona la rama predeterminada del repositorio. Luego se creará la rama requerida
...
TEMP_DIR=$(mktemp -d)
# Este mv ha sido la forma más sencilla de poder eliminar archivos que estaban allí
# pero ya no lo están. De lo contrario, tendríamos que eliminar los archivos de "$CLONE_DIR",
# incluyendo "." y con la excepción de ".git/"
mv "$CLONE_DIR/.git" "$TEMP_DIR/.git"

# $TARGET_DIRECTORY es '' de manera predeterminada
...
# Relacionado con https://github.com/cpina/github-action-push-to-another-repository/issues/64
git config --global --add safe.directory "$CLONE_DIR"
# Agregar la excepción para /github/workspace
git config --global --add safe.directory /github/workspace

if [ "$CREATE_TARGET_BRANCH_IF_NEEDED" = "true" ]
then
    echo "[+] Switch to the TARGET_BRANCH"
    # || true: si la $TARGET_BRANCH ya existía en el repositorio de destino:
    # ya es la rama actual y no se puede cambiar (no es necesario)
    # Si la rama no existía: cambia (crea) la rama
...
echo "[+] git diff-index:"
# git diff-index: para evitar que la confirmación de git falle si no hay cambios para confirmar
git diff-index --quiet HEAD || git commit --message "$COMMIT_MESSAGE"

echo "[+] Pushing git commit"
# --set-upstream: establece la rama al empujar a una rama que no existe
